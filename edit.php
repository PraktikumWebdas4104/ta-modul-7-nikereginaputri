...
$query = mysqli_query($koneksi, "SELECT * FROM db_mahasiswa WHERE nim = $nim"); 
...
<form method="POST">
    ...
    <tr>
        ...
        <tr>
        <td>Moto Hidup : </td>
        <td><textarea name="moto_hidup"><?php echo $ro['motto_hidup']; ?></textarea></td>
    </tr>
        <tr>
            <td></td>
            <td></td>
...
<?php
    if (isset($_POST['submit'])) {
        ...
        $asal = $_POST['asal'];
        $motohidup = $_POST['moto_hidup'];

        $qrySelect = mysqli_query($koneksi, "SELECT * FROM db_mahasiswa WHERE nim = '".$nim."'");
        $rw = mysqli_fetch_array($qrySelect);

        if ($prodi !== -1) {
            if ($fakultas !== -1) {
                $sql = mysqli_query($koneksi, "UPDATE `db_mahasiswa` SET `nama` = '".$nama."', `jk` = '".$jk."', `prodi` = '".$prodi."', `fakultas` = '".$fakultas."', `asal` = '".$asal."', `moto_hidup` = '".$moto_hidup."' WHERE nim = '".$nim."'");
